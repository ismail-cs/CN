...
float carPosX = -0.9f; // Car's initial position
float wheelAngle = 0.0f; // Initial rotation angle for wheels
float wheelSpeed = 2.0f; // Rotation speed for wheels

void drawCar() {
    // Car body
    glColor3f(0.0f, 0.8f, 0.0f); // Red color for the car body
    glBegin(GL_QUADS);
    glVertex2f(carPosX - 0.3f, 0.3f);
    glVertex2f(carPosX + 0.3f, 0.3f);
    glVertex2f(carPosX + 0.2f, 0.5f);
    glVertex2f(carPosX - 0.2f, 0.5f);
    glEnd();

    glColor3f(0.8f, 0.2f, 0.2f); // Red color for the car body
    glBegin(GL_QUADS);
    glVertex2f(carPosX - 0.7f, -0.055f);
    glVertex2f(carPosX + 0.7f, -0.055f);
    glVertex2f(carPosX + 0.6f, 0.3f);
    glVertex2f(carPosX - 0.6f, 0.3f);
    glEnd();
    ...
    glTranslatef(carPosX - 0.4f, -0.1f, 0.0f); // Translate to the position of the first wheel
    ...
    glPushMatrix(); // Save current matrix
    glTranslatef(carPosX + 0.4f, -0.1f, 0.0f); // Translate to the position of the second wheel
    glRotatef(wheelAngle, 0.0f, 0.0f, 1.0f); // Apply rotation to the wheel
...
void update(int value) {
    carPosX += 0.005f; // Move the car

    if (carPosX > 1.0f) {
        carPosX = -1.0f; // Reset car position when it reaches the right boundary
    }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Car Moving Left to Right in Loop");
